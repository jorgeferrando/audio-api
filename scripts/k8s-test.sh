...
TMPDIR_WIN=$(cygpath -w "$TMPDIR_LOCAL" 2>/dev/null || echo "$TMPDIR_LOCAL")
...
kubectl -n audio-api exec deploy/audio-api -- cat //tmp/k8s-test.mp3 > "$TMPDIR_LOCAL/input.mp3"
echo "OK: generated $(wc -c < "$TMPDIR_LOCAL/input.mp3") bytes"

echo "=== 4. Upload ==="
MSYS_NO_PATHCONV=1 curl -s -H "x-api-key: $API_KEY" -X POST "$API/audio" \
  -F "file=@$TMPDIR_WIN\\input.mp3;type=audio/mpeg" \
...
if [ -z "$TRACK_ID" ]; then echo "FAIL: no audioTrackId in response: $UPLOAD"; exit 1; fi
